...
func (*KeyShardPair) Descriptor() ([]byte, []int) {
	return fileDescriptor_b211296faad91a29, []int{0}
}
...
func (m *KeyShardPair) XXX_Merge(src proto.Message) {
	xxx_messageInfo_KeyShardPair.Merge(m, src)
}
...
func (*Wire) Descriptor() ([]byte, []int) {
	return fileDescriptor_b211296faad91a29, []int{1}
}
...
func (m *Wire) XXX_Merge(src proto.Message) {
	xxx_messageInfo_Wire.Merge(m, src)
}
...
func init() { proto.RegisterFile("expwire.proto", fileDescriptor_b211296faad91a29) }

var fileDescriptor_b211296faad91a29 = []byte{
		...
		for iNdEx := len(m.Pairs) - 1; iNdEx >= 0; iNdEx-- {
			{
				...
				i -= size
				i = encodeVarintExpwire(dAtA, i, uint64(size))
			}
			i--
			dAtA[i] = 0xa
		}
...
func (this *Wire) String() string {
	...
	repeatedStringForPairs := "[]*KeyShardPair{"
	for _, f := range this.Pairs {
		repeatedStringForPairs += strings.Replace(fmt.Sprintf("%v", f), "KeyShardPair", "KeyShardPair", 1) + ","
	}
...
func skipExpwire(dAtA []byte) (n int, err error) {
	...
	for iNdEx < l {
		...
		case 4:
			if depth == 0 {
				return 0, ErrUnexpectedEndOfGroupExpwire
			}
			depth--
		case 5:
			iNdEx += 4
